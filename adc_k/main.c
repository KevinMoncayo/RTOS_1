 ...
 *********************************************************************/
static void
temp_task(void *arg __attribute((unused))) {
	int temp100, vref, tempAmb;
	int adc0; //adc1;
		...
		std_printf("Temperature %d.%02d C, Vref %d.%02d Volts, VLM35 %d.%02d mV, Ambient Temperature %d.%02d C\n",
			temp100/100,temp100%100,
			vref/100,vref%100,
			adc0/1,adc0%1, //this is right value for temperature measurement
			tempAmb/1,tempAmb%1);
        }
}
...
main(void) {
	...
	xTaskCreate(temp_task,"temp",400,NULL,1,NULL); 
